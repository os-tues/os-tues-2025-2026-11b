...
- Чрез оператора за присвояване `променлива=[стойност]` /без празни пространства/.
    ...
    - Ако символният низ е заграден от апострофи (' ')  се показват всички символи заедно с интервали и табулации.

    - Ако символният низ е заграден с двойни кавички (" ")  се показват всички символи, като се подменя стойността на дадена променливa.
...
- Чрез командата `read`- `read [име/на променлива/и]`
    - Чете се един ред от стандартния вход и се разделя на думи (разделителят е интервал) като всяка дума се съпоставя на съответната променлива.
...
- В bash, всички променливи се приемат за символни низове (strings). Затова, аритметични изчисления се осъществяват се чрез командата `expr` или чрез `$((...))`.

- Изчислява се значението на подадения целочислен израз и резултатът се извежда на стандартния изход. Изразът се конструира от цели числа, променливи и целочислените аритметични операции: +, -, *, /, %.
...
## 7. Получаване на помощна информация

* **`man` (manual)** – Отваря системното ръководство за дадена команда (обикновено за външни програми и инструменти).
* Излиза се от режим на четене с бутона `q`.
* Навигацията става със стрелките или `Space` (за следваща страница).
* Пример: `man ls`, `man grep`

* **`help`** – Извежда помощна информация за **вградени** (built-in) команди на bash (такива, които са част от самия shell и нямат отделен изпълним файл).
* Пример: `help cd`, `help echo`, `help if`, `help read`

* **Опция `--help**` – Повечето стандартни команди поддържат тази опция за бърза справка на синтаксиса и възможните параметри, без да се отваря цялото ръководство.
* Пример: `mkdir --help`, `cp --help`