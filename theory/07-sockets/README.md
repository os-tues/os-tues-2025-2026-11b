...
3. [Мрежови протоколи - IPv4/IPv6](#3-мрежови-протоколи---ipv4ipv6)
...
6. [Модел сървър-клиент. Socket комуникация в C](#6-модел-сървър-клиент-socket-комуникация-в-c)
---
...
- Сравнение между TCP и UDP:
  ...
  - **UDP**: По-малка латентност, без гаранция за доставяне на данни, подходящ за приложения в реално време.
...
- `traceroute` - Командата `traceroute` служи за проследяване на маршрута, по който пакетите достигат до даден хост в мрежата. Тя показва всяка междинна точка (hop) през която минават пакетите, което помага да се идентифицират проблеми с мрежовите връзки.
    ...
    - Бележка: Не всички рутери по трасето отговарят на ICMP пакети, затова някои рутери са отбелязани с `* * *`.
...
3. Сървърът започва да слуша за входящи връзки посредством функцията `listen()`. Тази функция превключва сокета в режим на слушане и определя максималния брой връзки, които могат да бъдат наредени на опашка за обработка.
